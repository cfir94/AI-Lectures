...
- A video slide is the one deliberate exception, and it is scoped: opening the deck must still touch no network at all. The embed is created only when the presenter presses play, only for an ID matched against the known-source table (never a pasted URL), and the slide must stand on its own poster — stored in the document — when the room has no internet.
