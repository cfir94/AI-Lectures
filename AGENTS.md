...
- **The deck runs on four colours, not seven.** The palette table is the range available; a talk uses a closed set of it. This one runs on `ice` (the house: charcoal and cyan), `cobalt` (the deeper ground once the talk is about doing), `danger` (only where something breaks), and exactly one `amber` slide (the single human moment). A palette that appears once by accident reads as a mistake; a palette that appears once **on purpose**, in the same role every time, reads as a decision. The same discipline governs the other tables: most slides are `plain`, `rise` and `fade`, and every departure — a backdrop, a `cut`, a `push`, a `spectrum` headline — has to be able to say what it means. Variety spread evenly is not design, it is noise; before adding a look, count how many the deck already uses and what each one is for.
